...
6. **fail-closed 가드에는 복구 경로와 진단을 함께 만든다.** 거부는 안전하지만, 빠져나갈 길이
   없으면 사용자는 가드를 우회하는 법부터 찾는다 — 그 순간 가드는 사라진다. 이번 세션에만
   네 번 나왔다: 버전 게이트에 복구 명령이 없었고, 그 복구 명령이 자기 게이트에 막혔고,
   argv 가드가 정당한 값 플래그를 오판했고, 강제 범위(4필드)가 지시 범위(2필드)보다 넓어
   구조적으로 항상 실패했고, cwd를 홈 아래 고정하라는 조항과 상위 경로에 `.claude/`가 있으면
   거부하라는 조항이 **각각은 옳은데 교차하면 모든 환경에서 실패**했다(`$HOME/.claude`는 항상
   있다). **조항 하나씩 검토하면 이 유형은 안 잡힌다 — 가드와 그 가드가 검사할 값을 함께 놓고
   본다.** 거부할 때는 왜 거부했는지와 어떻게 푸는지를 함께 출력한다.
...
- **픽스처가 실제 응답 형태와 다르면 그 파서는 검증되지 않는다.** 봉투를 페이로드로 착각한
  버그를 테스트가 못 잡은 이유가 픽스처도 같은 착각을 하고 있어서였다.
...
Step 0~5 완료 — `ctk init → scan → measure → gen → usage`, `move → rollback`, 그리고
`toolkit-search` 진입 스킬(요구사항 8)이 동작한다. **AC-3.3(에이전트 자율 발견)은 미측정** —
합성 카탈로그는 `HOME` 덮어쓰기를, 인증은 `CLAUDE_CONFIG_DIR` 미설정을 요구해 설계가 모순이다
(`docs/harness-facts.md`). 남은 것은 웹 콘솔(Step 6a/6b)이다. 계획서는 `.omc/plans/`(gitignore).
