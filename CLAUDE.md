...
Step 0~6b 구현 완료 — 조회 콘솔(`ctk web`)과 액션 API(`--actions`: scan·rollback·move·gen)가
동작하고 `security-reviewer` 재심에서 High/Critical 0건을 받았다. 남은 것은 웹 UI의 액션 버튼과
후속 Low 정리다. **AC-3.3은 미측정**(설계 모순 — `docs/harness-facts.md`).
