...
6. **fail-closed 가드에는 복구 경로와 진단을 함께 만든다.** 거부는 안전하지만, 빠져나갈 길이
   ...
   구조적으로 항상 실패했다. **거부할 때는 왜 거부했는지와 어떻게 푸는지를 함께 출력한다.**
...
Step 0·1·2·5·3·4 완료 — `ctk init → scan → measure → gen → usage`와 `move → rollback` 왕복이
실제 환경에서 동작한다. 남은 것은 `toolkit-search` 스킬(요구사항 8)과 웹 콘솔(Step 6a/6b)이다.
계획서는 `.omc/plans/`(gitignore)에 있다.
