...
### 미해결 — `--disable-slash-commands`가 봉인 세션을 멈춘다 (2026-08-24, CLI 2.1.241)
...
**아직 원인 미상이다.** 코드 쪽 변경(트리 감사·에러 클래스)은 spawn 경로에 닿지 않으므로
배제된다. CLI 재시작·재로그인·시간 경과 후 재관측이 필요하다. 그때까지 `gen`은 문서를
생성하지 못한다.
