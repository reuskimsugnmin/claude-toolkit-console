...
**⚠️ `ctk agent-probe`는 지출을 아무 데도 남기지 않는다(2026-08-26 확인).** `cost`를 다루는
코드가 `gen/src/agent-probe.ts`와 CLI 출력부 **양쪽에 없다.** `gen`은 중단돼도 run-log를 쓰는데
(#24) 진단 경로는 성공해도 0이다. AC-3.3의 유료 진단 4회 지출도 그래서 어디에도 없다.
