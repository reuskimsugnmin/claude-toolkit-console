...
- **`gen`은 조건부로 따라간다(2026-08-24 정정)** — 예전에는 `lstat`으로 링크를 일률 거부했다.
  지금은 **스킬 원문에 한해** ① realpath가 스킬 발견 루트 중 하나 안이고 ② 대상의 파일명이
  링크와 같을 때만 따라간다. 플러그인 원문(README·plugin.json)은 여전히 일률 거부다.

즉 비대칭은 사라진 것이 아니라 **축이 바뀌었다** — "probe는 따라간다 / gen은 거부한다"가 아니라
**"probe는 무제한 따라간다 / gen은 봉쇄 루트 안의 같은 이름까지만 따라간다"**이다. `probe` 경로의
실제 노출은 `frontmatter.description` 한 필드로 한정되고(비-frontmatter 파일이면 값이 없다)
공격자가 이미 그 디렉터리를 통제하는 상황이라 위험은 낮지만, 속성을 일반 규칙처럼 적어두면 어긋난다.

**⚠️ 스킬 디렉터리 자체가 링크인 경우는 어느 쪽도 읽지 않는다** — `sources/skills.ts`의
`if (!dirent.isDirectory()) continue;`가 링크 dirent를 **발견 단계에서** 제외하기 때문이다
(`readdirSync`의 `withFileTypes`는 lstat 의미라 링크는 `isDirectory()=false`다).

**이것은 이제 부수 관찰이 아니라 디렉터리 축을 닫는 유일한 방어선이다.** `gen`의 위생 검사는
`SKILL.md` **파일 하나만** 보므로, `readSkillDir`이 언젠가 링크를 따라가게 바뀌면 봉쇄 검사는 그
축을 전혀 보지 못한다. `gen/test/plan.test.ts`가 이 성질을 고정한다 — 계층이 어긋나는 자리에
테스트를 둔 것은 의도적이며, 깨지면 봉쇄를 디렉터리 축까지 넓혀야 한다는 신호다.
...
### 스킬 경로 — 2026-08-26 측정 완료 (통과)

플러그인 경로만 통과였던 것을 스킬 경로까지 닫았다. **새 세션 요건은 헤드리스 `-p`로 만족한다** —
자기 참조를 피하는 핵심은 자동화 여부가 아니라 **판정을 `ctk`가 하지 않는 것**이다. 여기서는
`ctk`가 만든 문서에서 사람이 문구를 꺼내고, 판정은 하네스가 남긴 도구 호출 기록이 한다.

**절차**: 생성된 `annotation.md`의 「When to use」 절에서 상황 문장을 **그대로 복사**해
`claude -p`에 입력한다. 쓰기 계열 도구는 `--disallowed-tools`로 막는다 — AC-3.5는 "스킬이
로드되는지만 본다(끝까지 진행할 필요 없다)"이므로 실행 능력이 필요 없고, 실환경에 부작용을
남기지 않는다. 관측은 `--output-format stream-json`의 `tool_use`다.

| 항목 | 값 |
|---|---|
| 판정 근거 | `{"name":"Skill","input":{"skill":"<대상>"}}` — 하네스가 남긴 도구 호출 |
| 오발동 | **0건** — 로드된 스킬은 대상 하나뿐(`Base directory for this skill` 라인 1건) |
| 종료 | `exit=0` · `subtype: success` · 6턴 · 56.7초 |

**⚠️ 진단 1회가 $0.863이었다.** 두 축이 겹쳤다.
1. **모델** — `claude -p`에 `--model`이 없으면 자식이 **사용자의 기본 모델**을 쓴다. 여기서는
   `claude-opus-5[1m]`이 붙었다. `gen`은 이 함정을 이미 sonnet 고정으로 닫았는데(#23)
   **수동 진단 명령은 닫히지 않았다** — 같은 결함이 다른 자리에 남아 있었다.
2. **환경** — `cache_read 218,092 tok` · `cache_creation 67,799 tok`. 새 세션이 질문을 받기도
   전에 20만 토큰을 읽는다. 이것은 이 제품이 푸는 **문제 2(전역 설치가 많아 기본 컨텍스트
   비용이 과다하다)**가 실측으로 나타난 숫자다. **헤드리스 진단이 비싼 이유는 모델만이
   아니라 환경이다** — 진단 비용을 견적할 때 자산 크기만 보면 한 자릿수가 틀린다.

**⚠️ `ctk agent-probe`는 지출을 아무 데도 남기지 않았다(2026-08-26 확인 · 같은 날 수정).**
`cost`를 다루는 코드가 `gen/src/agent-probe.ts`와 CLI 출력부 **양쪽에 없었다.** `gen`은 중단돼도
run-log를 쓰는데(#24) 진단 경로는 성공해도 0이었다. AC-3.3의 유료 진단 4회 지출은 **지금도
알 수 없다.**

### 유료 spawn 지점은 셋이고 `--model`은 하나에만 있었다 (2026-08-26 실측)

`claude -p`를 띄우는 자리를 전수 조사한 결과다. **#23이 `gen`을 sonnet으로 고정할 때 나머지
둘은 그대로였다** — 한 자리를 고쳤을 때 같은 형태의 다른 자리를 세지 않은 결과다(안전 원칙 5).

| 지점 | 하는 일 | `--model` | 지출 기록 |
|---|---|---|---|
| `gen/src/index.ts` → `run-claude-p.ts` | 문서 생성 | ✅ (#23) | ✅ run-log |
| `gen/src/agent-probe.ts` | AC-3.3 진단 | ❌ → ✅ | ❌ → ✅ 화면 |
| `gen/src/seal-live-test.ts` (3호출) | 봉인 시험 | ❌ → ✅ | ❌ |

**`seal-live-test`의 누락은 릴리스 게이트를 죽일 수 있었다.** `pnpm release:seal`은
`--max-budget-usd 0.50`으로 고정돼 있는데, 자식이 사용자 기본 모델을 쓰면 상시 컨텍스트가 큰
머신에서 호출 하나가 그 상한을 넘는다(실측: 진단 1회 $0.863). 그러면 게이트가 **0원에 거부되고**,
그것은 "통과"가 아니라 **미측정**인데 그 구분이 사라진다.

**모델 상수는 셋으로 나눴다** — `GEN_MODEL`·`PROBE_MODEL`·`SEAL_TEST_MODEL`. 값은 같지만 뜻이
다르다("문서 생성에 충분한 모델" · "스킬 발동을 재는 기준선" · "봉인이 뚫렸는지 재는 데 충분한
모델"). 하나로 합치면 한쪽 사정으로 바꿀 때 다른 쪽 판정 기준이 조용히 따라 움직인다.

**진단 지출은 화면에 남긴다** — `agent-probe`는 "`sync`를 전혀 호출하지 않는다"가 계약이라
run-log에 쓸 수 없다. **계약이 저장 위치를 정하고, 저장할 수 없으면 표시가
유일한 자리다.** 읽지 못한 값은 "미보고"로 말하고 0원이라 하지 않는다(안전 원칙 7).

### AC-3.3 재검증 (2026-08-26 · R3의 파생)

AC-3.1 해소로 진입 스킬 `description`을 영어로 바꿨으므로(89 → 36토큰) 기존 통과는 **옛 문장에
대한 결과**였다. 같은 절차로 1회 재측정했고 **네 축 전부 통과**했다 — 발견(툴명·스킬명 없이
상황만 주고 자율 발동) · 설치 상태(지정 머신 기준 스코프와 연결 프로젝트) · 카탈로그 경계 ·
머신 식별. 36초 소요로 모델이 실제로 돌았음을 검산했다.

**새로 나온 지적**: 에이전트가 "이 자산은 project 스코프라 **지금 cwd에서는 발동하지 않을
가능성이 크다**"고 스스로 덧붙였다. 과거 4회 진단이 매번 새 결함을 냈던 것과 같은 자리인데
이번 것은 결함이 아니라 **설치 스코프와 활성 축을 정확히 구분한 답**이다. 픽스처 인덱스에
`gen_state`가 없어 신선도를 표시할 수 없다는 지적도 정확했다(픽스처의 결손).
...
## 링크 일률 거부를 봉쇄로 바꿀 때의 결함 3종 (2026-08-24 실측 · 보안 심사)

위생 검사가 심볼릭 링크를 **일률 거부**하던 것을 **봉쇄 루트 안이면 허용**으로 바꾸자 세 결함이
드러났다. 공통 뿌리는 URL 케이스와 같다 — **거부가 검사의 성김을 가려주고 있었다.**

**① 봉쇄 루트만 보고 파일명을 안 보면 서브트리 전체가 새어나간다.** `skills/evil/SKILL.md`를
`skills/victim/.env`나 `skills/.git/config`로 링크하면 그 내용이 카탈로그 문서에 박혀 저장소로
동기화된다(실증). 도트파일 관리자가 `skills/`를 git 저장소로 심는 배치는 흔하고, 그 remote URL에
토큰이 들어 있을 수 있다. **링크를 허용하기 전에는 공집합이던 위험**이다.

> 처방은 **파일명 축을 하나 더 거는 것**이다 — 해소된 대상의 basename이 링크 자신과 같아야 한다.
> 대가는 실측 0: 이 환경의 심볼릭 링크 `SKILL.md` **55건의 대상 basename이 100% `SKILL.md`**였다.
> 허용하려는 설치 방식은 "같은 이름의 파일을 다른 자리에 심는" 것뿐이다.

**② 봉쇄 루트를 한 자리만 배선하면 다른 스코프가 통째로 막힌다.** 스킬 발견 루트는
`<configDir>/skills` 하나가 아니라 **프로젝트마다 하나씩 더** 있다(`<프로젝트>/.claude/skills`).
user 루트만 넘기면 프로젝트 스코프 스킬의 링크가 전부 거부된다. **발견 루트와 봉쇄 루트는 같은
빌더에서 나와야 한다** — 따로 조립하면 발견 루트가 늘 때 봉쇄가 조용히 그 축을 놓친다.

**③ 봉쇄 루트의 ENOENT를 원문의 ENOENT와 섞으면 사유가 뒤바뀐다.** 루트가 없을 때
`realpathSync(root)`의 ENOENT가 원문 읽기 catch에 잡혀 `asset_source_missing`("원문이 사라졌다")로
분류됐다. 원문은 멀쩡하고 사라진 것은 **가드가 검사할 값**이다 — 화면은 사용자에게 있지도 않은
드리프트를 조사시킨다(안전 원칙 7).

**막지 못하는 것도 함께 적는다**(과장된 주석은 다음 사람이 그 위에 결정을 쌓는다):

| 축 | 상태 | 선재 여부 |
|---|---|---|
| `..` · 상대경로 · 중첩 링크 · 형제 접두 · 디렉터리 링크 | **막힘**(실증) | — |
| **TOCTOU** — 검사와 읽기가 경로를 각각 해소 | **뚫림** · 실측 우회율 11% | 선재(창이 넓어짐) |
| **하드 링크** — `realpathSync`가 되짚지 않는다 | **뚫림** | 선재 |

TOCTOU를 닫으려면 fd를 한 번 열어 `fstat` + fd 읽기로 묶어야 한다. 하드 링크는 `st.nlink > 1`
거부 축이 필요하며 정상 파일 오탐을 먼저 재야 한다. **둘 다 봉쇄 도입 이전에도 통과하던 경로다.**

---

## `claude` CLI가 올라가면 봉인이 스스로 멈춘다 — 복구 경로 실측 (2026-08-25)

배치를 띄우자 **0원에 1분 21초로 거부**됐다.

```
FAIL seal_unverified_cli: claude 버전이 검증된 버전과 다르고(검증: 2.1.241, 실제: 2.1.243)
0원 라우팅 신호 재현도 실패했다 — sealed-live 실행을 거부한다.
복구: ctk verify seal --installed-plugin-command <커맨드> --max-budget-usd <수치> --timeout-sec <초>
```

**모델은 한 번도 돌지 않았다** — 1분 21초라는 소요 시간이 그 검산이다(유료 실행은 시간으로 잰다).

### 왜 버전을 축으로 잡는가

봉인은 `--safe-mode`·`--tools ""`·`--disable-slash-commands` 같은 **플래그의 의미**에 의존한다.
그 의미는 버전 사이에 조용히 바뀔 수 있고, 플래그가 무시되면 **봉인은 통과한 것처럼 보이면서
실제로는 사라진다.** `claude`는 자동 업데이트되므로 `verified_cli_version`은 반드시 낡는다.

### 복구는 실제로 작동했다 — 그리고 전제가 하나 있었다

`ctk verify seal`이 ⓓ-2의 3신호를 새 버전에서 재현하고 **전부 통과했을 때만** 기록을 갱신한다
(9.75초 · 4신호 전부 통과 · 2.1.241 → 2.1.243).

⚠️ **통과의 전제는 (i)의 양성 대조군이 실환경에 심어져 있다는 것이다.** `settings.json`의
**SessionStart** 훅이 `<sealed-live cwd>/.ctk-seal-hook-marker`를 만들어야 하고, 없으면 (i)이
`unmeasured`가 되어 `passed`가 영원히 false다 — **복구 경로가 있는데 통과할 수 없는 상태**가 된다.
이 머신에는 그 훅이 배선돼 있었다(경로도 일치). **fail-closed 가드를 점검할 때는 그 가드가
검사할 값이 실제로 존재하는지 함께 본다**(안전 원칙 6).

**`pnpm release:seal`은 `--installed-plugin-command`가 빠져 있어 그대로는 실패한다.** 그 값은
설치된 툴 이름이라 public 저장소에 박을 수 없다 — 호출자가 인자로 덧붙여야 한다.

## 비용 견적이 **중앙값**으로 총액을 투사하고 있었다 (2026-08-25 실측 · 수정 완료)

`gen` 호출당 비용 분포는 **오른쪽으로 길다** — 최대가 중앙값의 **약 4.1배**다. 그런데 화면
둘(CLI `describeCostEstimate` · 웹 `ui-page.ts`)이 `중앙값 × 건수`로 총액을 투사했다.

배치 3회에서 `중앙값 × 건수`가 실제 총액의 몇 %였는지:
...
> 절대 금액·건수는 이 저장소가 public이라 담지 않는다 — 원본은 private 운영 저장소에 있다.

**합계는 건수 × 평균이다.** 그리고 평균은 이미 저장돼 있었다 —
`reported_total_usd / calls_reported`. 스키마를 늘릴 필요가 없었고 곱하는 값만 틀렸다.

**개별 값이 전부 정확해도 집계는 거짓 결론을 낸다**(안전 원칙 8). 중앙값도 최대값도 각각
맞았다. 그리고 **두 표시 계층이 똑같이 틀려 있었다** — 그래서 고칠 때 각각 고치지 않고
투사를 `core/view/gen-cost-projection.ts` 하나로 모으고 웹은 서버가 계산한
`projected_total_usd`를 받아쓰게 했다. **표시가 산술을 하면 결함이 화면 수만큼 갈라진다.**

중앙값은 버리지 않는다. 둘은 다른 질문에 답한다 — **중앙값은 호출당 상한을 정할 때**,
**평균은 배치 총액을 볼 때** 쓴다.

## 봉인 신호 (ii)가 호출 실패를 "부재"로 삼켰다 (2026-08-25 · 수정 완료)

```ts
const claudeMdStringAbsent = !(claudeMdCheck.exitCode === 0 && containsYes(stdout));
```

타임아웃·봉인 에러·인증 실패 무엇이든 **호출이 실패하면 곧바로 `absent = true`**가 되어
신호가 통과로 읽혔다. **"없음"과 "실패"를 구분한다**(안전 원칙 7).

양성 대조군이 이걸 **부분적으로만** 막는다 — 대조군은 "탐지 자체가 가능한가"를 증명할 뿐
**이번 호출이 성립했는가**는 말해 주지 않는다. (i)은 이미 3상태였는데 (ii)만 boolean이었다.

수정: `"confirmed_absent" | "present" | "unmeasured"` 3상태. YES도 NO도 아닌 응답 역시
`unmeasured`다 — 판정할 수 없으면 판정하지 않는다.

## 락 신호 핸들러가 누적되고 남의 핸들러까지 지웠다 (2026-08-25 · 수정 완료)

전날 넣은 신호 핸들러(SIGKILL로 남은 락을 막으려던 조치)에 결함 둘이 있었다.

1. **정상 해제가 신호 핸들러를 떼지 않았다.** `release()`는 `exit`만 걷었다. 락을 반복
   취득하는 프로세스(웹 서버 — 액션마다 취득)에서 리스너가 3개씩 쌓인다. 테스트 전량 실행에서
   `MaxListenersExceededWarning: 11 SIGTERM listeners`로 드러났다. `gen`은 실행당 1회라 무사했다.
2. **`process.removeAllListeners(sig)`가 범위를 넘었다.** 자기 재귀를 막으려는 의도였지만
   (그건 `process.once`가 이미 한다) **다른 코드가 등록한 핸들러까지 지웠다** — 장수 프로세스의
   graceful shutdown 핸들러가 그렇게 사라진다.

### 파괴 실험에도 축이 있다 — 이 결함이 처음엔 통과했다

②를 되살리는 파괴 실험이 **통과했다.** 테스트 셋이 전부 *정상 해제* 경로만 태웠고,
`removeAllListeners`는 **신호가 실제로 발화할 때만** 실행되기 때문이다.

신호 발화 축을 태우려면 재발화(`process.kill`)를 주입할 수 있어야 한다 — `terminateFn` 이음매를
넣고 `process.emit("SIGTERM")`으로 리스너만 돌려 잰다. 이음매 없이는 **그 축을 아예 태울 수 없다.**

## 검증 사례 아카이브 — `CLAUDE.md`에서 옮긴 것 (판단 기준은 그쪽, 사례는 여기)

`CLAUDE.md`는 200줄 상한이라 **판단 기준만** 남기고 사례는 여기로 온다. 아래는 그 기준들이
어떤 실측에서 나왔는지의 기록이다.

### 테스트가 코드가 아니라 환경을 단언하고 있었다

- cwd 가드 6종이 **실제 기본 cwd를 한 번도 보지 않았다.**
- "크레덴셜 없음"을 `env` 삭제로 재현하던 테스트는 머신에 크레덴셜이 생기자 **실제 네트워크를
  탔다**(23.75초 — 소요 시간이 아니었으면 못 알아챘다).

→ **부재를 단언하려면 부재를 주입한다.** 픽스처가 결과를 지배하지 못하면 통과는 코드가 아니라
그 머신에 대한 진술이다.

### 리뷰·재심 지적을 그대로 넣었다가 정상 동작이 위반이 됐다

보안 재심이 "개수 델타 둘을 함께 fail-closed로 올리라"고 했고 그대로 넣자 **정상 churn이
위반으로 판정**됐다. 계층이 다르면 같은 신호의 의미도 다르다.

→ **리뷰 지적도 검증 대상이다.** 받되 범위를 잰다.

### 게이트가 표준 명령에 등록되지 않아 한 번도 실행되지 않았다

한 세션에만 세 번이었다 — managed 정책 · ⓓ-2 봉인 · AC-3.9 LLM. 전부 계획서에는 있었고
`pnpm verify`에는 없었다.

→ **게이트는 만들 때 *동시에* `pnpm verify`에 등록한다.**

## 위생 게이트는 "개인 사용량 수치" 축을 보지 않는다 (2026-08-25 · 심사 L-2)

`scripts/hygiene-check.mjs`가 잡는 것은 **패턴화할 수 있는 것**뿐이다 — 홈 절대경로(macOS·Linux의 사용자 홈 접두사), 이 머신의 실제 `$HOME`·hostname 리터럴. 스크립트 주석이 스스로 적어 둔다:

> 개인 사용량 수치·설치 목록처럼 패턴화할 수 없는 항목은 [검출하지 않는다]

그래서 이번 세션에 문서·소스 주석·**테스트 픽스처**로 들어간 절대 지출액·설치 규모(자산 수·MB)가
**"위반 0건"을 그대로 통과했다.** `CLAUDE.md`의 금지 목록에 명시돼 있는데도다.

**게이트 통과를 위생 통과로 읽지 않는다** — 축이 어긋나면 정확히 통과한다. public 저장소에
숫자를 적을 때는 게이트가 아니라 **직접** 다음을 묻는다:

1. 이 값이 **제품 동작**을 말하는가, **이 사용자의 환경**을 말하는가?
2. 결론을 지탱하는 것이 **절대값**인가 **비율**인가? (대개 비율이다)
3. 테스트 픽스처인가? — 픽스처는 **합성이어야 한다**(`CLAUDE.md` 명시). 모양만 실측을
   따르게 하고 값은 지어낸다. 실측 원본은 private 운영 저장소에 둔다.

정리 기준(2026-08-25 적용): 오차율·분포 모양·판단 기준은 남기고, 절대 금액·설치 건수·바이트
규모는 비율이나 상대표현으로 바꿨다. 원본 표는 `../claude-toolkit-ops/reports/`에 있다.

## 고친 결함이 가리던 결함이 드러난다 (2026-08-25 · 심사 M-1)

락 신호 핸들러의 `process.removeAllListeners(sig)`는 **남의 핸들러까지 지운다**는 이유로 걷어냈다.
그 수정 자체는 옳았다. 그런데 그것이 **다른 결함을 가리고 있었다.**

Node는 신호에 **리스너가 하나라도 있으면 기본 종료를 적용하지 않는다.** 예전 코드는
`removeAllListeners`로 재발화 시점에 리스너를 0개로 만들었고, 그래서 프로세스는 **반드시 죽었다** —
즉 "신호를 받았는데 아직 살아 있는 프로세스"라는 상태가 **존재할 수 없었다.**

남의 핸들러를 보존하도록 고치자 그 상태가 처음으로 가능해졌고, 그 상태를 전제하지 않은 코드
둘이 깨어났다:

1. 신호 핸들러가 `released`를 세우지 않는다 → 뒤늦은 `release()`가 다시 지운다
2. 지울 때 **그 락이 우리 것인지 보지 않는다**

실증된 귀결 — A가 신호를 받고 락을 지운 뒤 살아남고, B가 락을 잡고, A의 뒤늦은 `release()`가
**B의 락을 지우고**, C까지 들어온다. **동시성 가드가 완전히 사라진다.**

### 무엇을 해야 하는가

**수정으로 새로 가능해진 상태를 열거한다.** "이 수정이 무엇을 고치는가"만 묻지 말고
**"이 수정 뒤에 이전에는 불가능하던 어떤 상태가 생기는가"**를 함께 묻는다. 특히 옛 코드가
과잉으로 무언가를 막고 있었다면, 그 과잉이 **다른 코드의 암묵적 전제**였을 수 있다.

파괴 실험도 이 축을 따로 봐야 한다 — 두 축(`released` · 소유권)을 각각 주입해 보니
**주 시나리오는 `released`만으로 막혔다.** 소유권 축을 단독으로 재려면 "락 파일이 그 사이 남의
것으로 바뀐" 입력을 따로 만들어야 했다. **두 축을 함께 닫으라는 권고는 테스트에도 적용된다.**

## `~/.claude/plugins/` 아래에는 최소 네 개의 독립 클록이 돈다 (2026-08-25 실측)

봉인 트리 감사가 **실제로 켜진 첫 배치**에서 두 번 연속 중단됐다
(`plugins/known_marketplaces.json` · `plugins/installed_plugins.json`). 조사하며 알게 된 것들이다.

### 클록 넷

| # | 주체 | 주기 | 남기는 것 |
|---|---|---|---|
| 1 | 호출당 | `claude -p` 1회 | `sessions/<pid>.json` — 허용목록에 있다 |
| 2 | **마켓플레이스 자동 갱신** | 약 41분(2표본) | 아래 참조 |
| 3 | **리스 스윕** | 별도 주기 | `plugins/.last_inuse_sweep`(ISO 문자열 1줄) |
| 4 | 사람·다른 프로세스 | 비주기 | `history.jsonl` · `.in_use/<pid>` |

**세션 uuid 축(`SESSION_OWNED_NOT_ATTRIBUTABLE`)은 3·4에 닿지 못한다** — 뒤에 나오는 리스 파일이
**pid 키**이기 때문이다.

### 마켓플레이스 자동 갱신 — 시간 트리거이고, 호출당이 아니다

- 등록된 마켓플레이스 **전체가 아니라 일부만** 갱신된다. **공식 마켓플레이스는 `autoUpdate`
  기본값이 true**이고 서드파티는 false다 — 그래서 `known_marketplaces.json`에 `autoUpdate` 필드가
  **없는 항목도 갱신된다**(부재 = 기본값). 이 필드만 보고 판단하면 틀린다.
- 갱신 대상들의 `lastUpdated`가 **1~2초 안에 연속으로** 바뀌고, 이어서 `installed_plugins.json`이
  일괄 재작성된다(엔트리 다수의 `lastUpdated`가 함께 바뀐다).
- 마켓플레이스 실체는 두 종류다 — **git 클론**과 **GCS 추출물**(`.gcs-sha` 파일이 마커).
- 갱신 경로는 `plugins/cache/temp_git_<epochms>_<rand>/`에 스크래치 클론을 만든다. **고아가
  남는다**(`.orphaned_at` 마커 포함) — 실측에서 십수 개가 잔존했다.

### ⚠️ 일어난 쓰기를 감사가 못 본 사례

git 백업 마켓플레이스의 `.git/FETCH_HEAD`가 **감사 창 안에서 실제로 다시 쓰였다**(mtime이 그
초를 가리킨다). 그런데 위반으로 잡히지 않았다 — `git fetch`가 새 커밋을 못 찾으면 `FETCH_HEAD`
**내용이 같아** 해시가 안 바뀌기 때문이다.

**즉 그날의 "위반 2건"은 커버리지가 아니라 운이다.** 원격이 한 커밋이라도 움직였다면
`.git/objects/**` · refs · 체크아웃 트리 · `cache/<새 버전>/**`까지 수십~수백 건이 터졌을 것이다.
**트리 감사의 수집 범위는 충분하다** — `probe/tree-collect.ts`의 `walk()`에는 경로 제외가 하나도
없고 심볼릭 링크만 건너뛴다. 안 잡힌 이유는 범위가 아니라 **내용이 같았기** 때문이다.

### 플러그인 캐시 리스 — pid 키라서 기존 귀속 축이 닿지 않는다

```
plugins/cache/<마켓플레이스>/<플러그인>/<버전>/.in_use/<pid>
   내용: {"pid":N,"procStart":"<ctime 문자열>"}
```

- 플러그인을 읽는 **모든** `claude` 프로세스가 시작 시 플러그인당 하나씩 쓴다.
- 실측에서 **수천 개**가 누적돼 있었고 한 달 전 것도 남아 있었다 — 스윕이 즉시 회수하지 않는다.
- **파일 추가·삭제로 나타나므로 트리 diff에서 곧바로 위반이다.** 그리고 `SESSION_OWNED_NOT_
  ATTRIBUTABLE`에 넣을 수 없다 — 그 축은 전부 세션 **uuid** 정규식인데 이건 **pid**다.
- 봉인 자식은 `--safe-mode`라 플러그인을 안 읽어 리스를 쓰지 않는다. 그래서 지금까지 조용했다.
  **배치 중에 터미널을 하나만 더 열어 `claude`를 띄우면 즉시 터진다.**
- `.in_use`가 비면 빈 디렉터리가 되는데, 감사는 `emptyDirCount`를 판정에서 의도적으로 뺐다
  (오탐 근거가 주석에 있다). 그 전이는 **`unmeasured`이지 clean이 아니다.**

### ⚠️ 등재해 놓고 한 번도 도달하지 못한 항목 — `.session-stats.json` (2026-08-26 실측)

`SESSION_OWNED_NOT_ATTRIBUTABLE`에 `exact: ".session-stats.json"`으로 **등재돼 있었는데
한 번도 제외되지 않았다.** 살아 있는 Claude Code 세션이 있으면 `gen`이 항상
`whitelist_violation`으로 중단됐다.

**관측 방법**: 세션을 켠 채 `ctk gen --max-assets 3`을 돌렸다. 98초 만에 중단됐고 메시지가
위반 경로를 그대로 실어 보냈다 — `[{"path":".session-stats.json","status":"violation"}]`.
같은 실행의 run-log에 `session_owned_excluded: 1`이 있어 **제외 기구 자체는 살아 있는데 이
항목만 못 걸렀다**는 것이 드러났다.

**원인**: 제외 판정이 `sessionUuidOf()` 하나였고, 그 함수는 두 질문에 동시에 답하고 있었다 —
"세션 소유인가"와 "어느 세션인가". 후자를 **경로 정규식**으로 뽑으므로, 경로에 uuid가 없는
항목은 전자에서 통과하고도 후자에서 `undefined`가 되어 제외 대상에서 빠졌다.
`.session-stats.json`은 uuid를 **파일 내용의 키**로 담는다(등재 주석에 이미 그렇게 적혀
있었다). **위 두 사례와 같은 축 문제인데 방향이 반대다** — 리스 파일(pid 키)은 축이 닿지
않아 **과잉 차단이 되는 줄 알았고**, 이건 축이 닿지 않는데 **등재돼 있어 막힌 줄도 몰랐다.**

**파급**: 규칙을 등재하는 것과 규칙이 동작하는 것은 다르다. 이 항목에는 **테스트가 하나도
없었다** — 트리 감사 테스트 33건 중 `.session-stats.json`을 태우는 것이 없었고, 그래서
전량 통과하는 동안 죽어 있었다. **허용목록에 항목을 넣을 때 그 항목을 실제로 태우는 테스트를
같이 넣는다.**

**처방**: 규칙마다 **어떤 축으로 귀속을 인정하는지**를 필수 필드 `attribution`으로 선언한다.
- `path_uuid` — 경로가 uuid를 담는다. `before`에 이미 있던 uuid만 인정하고 **append 검사**를 건다.
- `preexisting_file` — 경로에 uuid가 없다. `before`·`after` **양쪽에 있을 때의 수정만** 인정한다
  (신규 생성·삭제는 위반으로 남는다). 하네스가 통째로 다시 쓰므로 **append 검사를 걸지 않는다** —
  같은 판정 함수를 두 축이 쓰면 각 축에서 그 규칙이 옳은지 따로 물어야 한다.

**⚠️ 수용된 잔여 위험 — 이 축에는 내용 제약이 없다.** append도 크기도 쓸 수 없으므로, 봉인 자식이
`.session-stats.json`을 임의 내용으로 바꿔도 감사가 침묵한다. 받아들인 근거는 **소비자**다 — 이
파일을 읽는 것은 하네스뿐이고 `ctk` 코드에는 참조가 없다(2026-08-26 grep 확인). 트랜스크립트와
달리 `ctk measure`의 입력이 아니라서 위조 → 오프머신 전송·fail-closed DoS·집계 오염 경로가
성립하지 않는다. **이 파일을 읽는 `ctk` 코드가 생기면 판단을 다시 한다.** 테스트로 고정해 뒀다.

**삭제는 미측정이다.** 같은 목록의 `hooks/state/<uuid>.start`는 삭제가 정상 동작임이 실측됐으므로
이 파일도 그럴 여지가 있다. 판정할 수 없으므로 안전한 쪽(위반)을 택했다 — 실환경에서 이 경로로
중단되면 **그것이 실측 데이터**다.

**⚠️ 고친 결함이 가리던 것이 드러났다 — 그리고 실환경 축은 아직 못 닫았다(2026-08-26).**
수정 뒤 다시 돌리자 `.session-stats.json`은 위반에서 사라졌는데 **`hooks/logs/notifications.log`가
새로 나왔다.** 트리 감사는 첫 위반에서 멈추므로 앞의 것이 막고 있는 동안 뒤의 것은 **관측조차
되지 않았다** — 수정 실패가 아니라 성공의 증거다. **새로 가능해진 상태를 열거해야 한다.**

그런데 같은 실행의 `session_owned_excluded`가 **1로 그대로였다.** 이 값은 "제외 대상으로 매치된
수"가 아니라 **실제로 달라진 것**만 센다. 관측을 방해하지 않으려고 실행 중 아무 명령도 돌리지
않았더니 세션 활동이 없어 `.session-stats.json`이 **아예 갱신되지 않았다.** 즉 **결함이 태어난
조건이 재현되지 않았고, 수정은 유닛 43건으로만 검증된 상태다**(실환경 축 미측정).
**관측 창이 사건 주기를 덮는지 먼저 확인해야 한다** — 여기서는 관측을 조용히 만드는 행위가
사건 자체를 없앴다. 재려면 실행 중에 **의도적으로** 세션 활동을 만들어야 하는데, 그러면 다른
churn이 위반을 내므로 두 요구가 맞선다. 아직 방법이 없다.

**✅ 순수 터미널에서는 1회에 통과했다(2026-08-26).** Claude Code를 종료하고 돌리자 봉인 감사가
그대로 통과했고 3자산이 한 배치에 끝났다(실측 $0.21 · 자산당 중앙값 $0.067 · 인젝션 후검증
실행명령 4건). **막고 있던 것이 대상 자산이 아니라 부모 세션이었음이 대조로 확정됐다** —
같은 명령, 같은 대상, 세션 유무만 다른 두 실행이 갈렸다. **권장 절차는 여전히 세션 종료다.**

**살아 있는 세션에서 위반 파일은 시도마다 바뀐다** — 실측 2회에서 각각 다른 파일이 나왔고 매
시도가 유료다($0.05~0.08). 항목을 하나씩 등재하는 것은 두더지 잡기다(이 문서의 「클록 넷」이 이미
pid 키 리스 파일 등 **닿지 않는 축**을 예견해 뒀다). **순수 터미널(Claude Code 종료)에서 돌리는
것이 여전히 정답이고**, `--allow-concurrent-sessions`는 그 대가(감사 무력화)를 알고 쓰는 탈출구다.

**부류를 타입으로 닫았다(재심 M2).** 축을 필수 필드로만 두면
`{ exact: "foo.json", attribution: "path_uuid" }`가 그대로 컴파일되는데 그것이 **이번 결함과
동형**이다. 판별 유니온으로 `path_uuid`는 `pattern`만, `preexisting_file`은 `exact`만 받게 했다.
파괴 실험으로 확인 — 잘못된 조합 3종(exact+path_uuid · 매처 없음 · pattern+preexisting_file)이
전부 컴파일에서 걸리고, 세 번째 축을 추가하면 **판정 두 자리가 동시에** 깨진다(TS2322 · TS2366).
한 자리만 고치면 다른 자리가 조용히 면제된다.

### 그래서 처방은 허용목록 항목 하나가 아니다

**쓰는 주체를 열거해 닫으려는 시도는 실패한다** — `gen/tree-audit.ts`가 이미 그렇게 적어 뒀고,
이번에 그 예언이 두 번째로 맞았다(2026-08-24는 세션 uuid 축, 오늘은 uuid도 pid도 아닌 하네스
자체 타이머). 클록이 하나 더 발견될 때마다 같은 PR을 쓰게 된다.

**막을 수 있는 것은 막고**(자식의 자동 갱신을 env로 끈다 — 능력을 주는 것이 아니라 빼앗는다),
**막을 수 없는 것은 귀속으로 등급화**하는 방향이 뿌리에 가깝다. 다만 등급화는 "위반을 위반이
아니게 만드는 스위치"이므로, 등급을 낮출 수 있는 경로를 **열거로 고정하고 타입으로 강제**해야
한다(패턴 확장 금지) — 그러지 않으면 다음 위반도 그리로 흘려보내려는 압력이 생긴다.

## `subagent_type`은 두 형태뿐이다 (B4-b 실측, 2026-08-28)

`Agent` tool_use의 `input.subagent_type` 값은 **`bare_name`과 `plugin_qualified` 둘뿐**이다.

| 형태 | 건수 | 비율 |
|---|---|---|
| `bare_name`(콜론 0) | 213 | 64.7% |
| `plugin_qualified`(콜론 1 · 세그먼트 정확히 2) | 116 | 35.3% |
| 콜론 2개 이상 | **0** | — |

- **관측 방법**: 이 머신의 트랜스크립트 810개 파일을 전수 순회해 `type: "tool_use"` · `name: "Agent"`
  블록의 `input.subagent_type`을 세었다(표본 329건 · 고유값 19개). 공백·`/`·`@`를 담은 값은 0건이다.
- **`Skill` 도구 입력과 같은 축이다** — `classifySkillInput`(plugin_qualified 22 / bare_name 15)이
  이미 같은 두 형태를 상수로 박아 두었다. 같은 이름·같은 구분자(`:`)를 쓴다.
- ⚠️ **`plugin_qualified`의 앞 세그먼트는 플러그인 *이름*이지 자산 id가 아니다.** 카탈로그의 번들
  에이전트 id는 `<이름>@<마켓플레이스>:agent:<이름>`(B1 D2)이므로 **두 값은 형태가 달라 절대
  일치하지 않는다.** 이것이 `subagent_attribution`의 `"resolved"`가 한 번도 생산된 적 없던 이유다.
- **파급**: 이름으로 되짚으므로 **같은 이름의 플러그인이 두 마켓플레이스에 있으면 후보가 둘이 된다.**
  "플러그인 이름은 유일하다"는 **미측정 전제**이므로 리졸버는 그것에 기대지 않고 후보가 정확히
  하나일 때만 잇는다(`core/usage/subagent-resolve.ts`). `bare_name` 64.7%는 대부분 하네스 내장
  에이전트라 후보 0건으로 남는다 — **그것이 정상이고 결함이 아니다.**
- ⚠️ **콜론 2개 이상이 0건이라고 첫 콜론으로 가르지 않는다.** 그러면 예상 밖 형태가 조용히
  매칭 실패로 삼켜진다. 세그먼트가 정확히 둘일 때만 취급한다(미측정 전제에 기대지 않는다).

## 플러그인 `.mcp.json`은 두 형태가 공존한다 (B1 인터뷰 실측, 2026-08-26)

`.mcp.json`을 가진 플러그인 중 **약 30%가 `mcpServers` 래퍼 없이 서버명을 최상위 키로 둔다**
(`{ "<서버명>": { "command": ..., ... } }`). 나머지 약 70%는 `{ "mcpServers": {...} }`다.

- ⚠️ **원래 적혀 있던 관측 방법이 틀렸다**(2026-08-28 정정). "최상위 키가 `mcpServers`
  하나뿐인지 센다"는 **래퍼형인데 추가 최상위 키를 가진 파일을 평면형으로 오분류한다.**
  올바른 판별은 **`mcpServers` 키가 있고 그 값이 객체인가**이고, 나머지 최상위 키는 서버가 아니다.
- **재측정(2026-08-28, B4-a 착수 전 · 설치 플러그인 66개 전수)**

  | 관측 | 값 |
  |---|---|
  | `.mcp.json` 보유 파일 | **17** |
  | └ 래퍼형(`mcpServers` 키 보유) | **14** — 그중 **3건은 추가 최상위 키를 함께 갖는다**(실측: `recommendedCategories`) |
  | └ 평면형(`mcpServers` 없음) | **3** |
  | 평면형이 `mcpServers` 키를 가진 경우 | **0건** — 판별 규칙이 이 머신에서 애매해지지 않는다 |
  | 최상위 값이 객체가 아닌 항목 · 파싱 실패 | **0건** |
  | **총 서버 정의 수** | **23** (파일 수 17과 다르다 — 한 파일이 여러 서버를 담는다) |
  | 같은 순회의 `hooks/` 보유 플러그인 | **16** (B4-a-2 판단 재료) |

- ⚠️ **파급이 바뀌었다.** 예전 서술은 "`mcpServers` 키만 읽는 파서는 그 30%에서 조용히 0건을
  낸다"였다. 실제 위험은 그 반대쪽이 더 크다 — **틀린 판별 규칙을 쓰면 래퍼형 3건에서
  `mcpServers`·`recommendedCategories`를 *서버 이름으로 읽어* 쓰레기 자산을 만든다.**
  "0건"(누락)뿐 아니라 "허위 건수"(과잉)도 같은 규칙에서 나온다.
- **관측이 도구를 의심하게 만든 사례다** — 정본에 적힌 방법을 그대로 쓴 첫 측정이
  "래퍼 11 / 평면 6"을 냈고, 판별을 고쳐 다시 재자 "래퍼 14 / 평면 3"이었다. **코드를 짜기 전에
  쟀기 때문에 파서가 아니라 문서에서 잡혔다.**
- **파급**: **`mcpServers` 키만 읽는 파서는 그 30%에서 조용히 0건을 낸다.** "없음"과 "실패"가
  같아진다(안전 원칙 7). 파서를 만들 때는 **두 형태를 가르고, 어느 쪽도 아니면 에러를 던진다.**
- 현재 ctk는 번들 MCP를 Asset으로 만들지 않으므로(「결정 7」) 아직 이 파서가 없다. **B4 대상.**

## 플러그인 번들 구성물은 5종이다 (B1 인터뷰 실측, 2026-08-26)

`skills/` · `commands/` · `agents/` · `hooks/` · `.mcp.json`.

- **관측 방법**: 활성 플러그인의 `installPath` 최상위를 전수 열거한다.
- **구조가 서로 다르다** — 스킬은 `skills/<이름>/SKILL.md`(디렉터리), 커맨드는 `commands/*.md`
  (평면), 에이전트는 `agents/*.md`. **하나의 워커로 추상화하면 건수가 어긋난다.**
- **커맨드의 중첩 디렉터리 이름 규약은 `unmeasured`다.** 중첩 사례가 존재하지만 그 항목이
  어떤 이름을 갖는지는 재지 않았다. **미측정은 통과가 아니다** — 편입 건수 판정에서 "아직
  실패일 수 있음"으로 센다.
- **파급**: ROADMAP은 오랫동안 3종(스킬·MCP·커맨드)만 적고 있었다. 에이전트와 훅이 빠져
  있었고, 그중 에이전트는 사람이 명시적으로 스폰하는 것이라 제품 문제 1의 정면 대상이다.

## `installed_plugins.json`의 `installPath`를 조립으로 대체하면 고아를 센다 (2026-08-26 실측)

| 관측 | 값 |
|---|---|
| `installPath`가 디스크에 실재 | **100%** |
| 마지막 세그먼트가 선언 `version`과 일치 | **100%** |
| 캐시에 버전 디렉터리가 둘 이상인 플러그인 | 약 15% |
| 버전 이름이 semver가 아닌 경우(해시·문자 그대로 `unknown`) | 약 20% |
| 그중 `installed_plugins.json`이 참조하는 것 | **0건 — 전부 고아** |

- **관측 방법**: 선언된 `installPath` 전량을 `existsSync`로 확인하고, 캐시 트리를 따로 훑어
  선언에서 참조되지 않는 버전 디렉터리를 센다.
- **파급**: `<cache>/<마켓>/<이름>/<버전>`을 **조립하면** ⓐ 문법 없는 세그먼트를 추측해야 하고
  ⓑ **고아를 설치된 것처럼 세어 편입 건수를 과대 계상한다.** 경로는 선언값을 그대로 읽는다.
- ⚠️ **그 대신 선언값은 검증 없이 들어온다** — 스키마가 `z.string()` + `.passthrough()`라
  절대경로인지도 `..`를 담는지도 보지 않는다. 그 값이 `readdirSync`의 루트가 되는 순간
  신뢰가 승격되므로 **`isAbsolute` + `realpath` 소속 확인을 순회 전에** 한다.

## 유니온에 값을 하나 더해도 컴파일이 깨지는 곳은 소수다 (2026-08-26 실측)

`AssetKind`(당시 4값)에 프로브 값 하나를 넣고 전체 `tsc -b`를 돌린 결과:

- 소비 지점 **23곳 중 컴파일 에러 3곳**(약 13%). 나머지는 **조용히 흘린다.**
- 걸린 셋: `Record<AssetKind, number>` 객체 리터럴 · exhaustive `switch` + 반환형에 `undefined`
  없음 · **손으로 베낀 4-리터럴 유니온을 받는 함수의 호출 인자**.
  ⚠️ 마지막 것은 **에러가 베낀 유니온의 선언부가 아니라 호출부에 뜬다.**
- **닿지 못하는 축이 있다** — HTML 문자열로 박은 `<option value="...">`는 타입 축이 전혀
  닿지 않는다. 그것은 런타임 테스트로 태워야 한다.

**파급**: `filter().length`·`if (kind === "x")`·경로 문자열 보간은 새 값을 전부 통과시킨다.
유니온이 자랄 예정이면 **소비 지점을 세고, 통과하는 것을 exhaustive 형태로 옮긴 뒤,
주입 게이트를 표준 명령에 등록한다.**

## pnpm 워크스페이스 사본에 타입을 주입할 때 게이트가 조용히 무효가 된다 (2026-08-26 실측)

저장소를 임시 디렉터리에 복사해 스키마에 프로브 값을 주입하고 `tsc -b`를 돌리는 게이트에서,
**패키지별 `node_modules`를 원본으로 심볼릭 링크했더니 주입이 아무 효과도 내지 않았다.**

| 시도 | 결과 |
|---|---|
| 패키지별 `node_modules` → **원본으로 심볼릭 링크** | `tsc -b` **EXIT=0 · 에러 0줄** |
| 패키지별 `node_modules` → **링크 구조를 `cp -R`로 복사** | **EXIT=2 · 에러 5줄 / 3파일** |

- **원인**: 워크스페이스 내부 링크가 `@ctk/<pkg> -> ../../../<pkg>`라는 **상대 심볼릭 링크**이고,
  상대 경로는 **심볼릭 링크의 실제 경로 기준**으로 해석된다. `node_modules` 전체를 원본으로
  링크하면 그 상대 경로가 **원본 트리로 되돌아간다** — **사본이 원본을 타입체크한다.**
- **파급**: 게이트가 "통과"를 보고하지만 아무것도 검사하지 않았다. **통과는 "검증됨"이 아니다.**
- **처방**: 게이트는 실행 전에 **양성 대조군**을 스스로 확인한다 —
  `realpath(사본/packages/<x>/node_modules/@<scope>/<y>)`가 **사본 안**을 가리키는지 단언하고,
  주입 전 기준선이 **에러 0**인지 확인한다. 둘 중 하나라도 아니면 게이트는 즉시 실패한다.
- 참고 비용: 이 저장소 전체 `tsc -b`는 사본에서 **약 2초**다. 게이트를 `verify`에 넣어도 싸다.

## `SKILL.md`가 FIFO면 `readFileSync`가 영구 블록된다 (B1 보안 심사 3차 M-1, 2026-08-27 실측)

번들 스캐너 둘(`scanBundledSkills`·`scanFlatMdKind`)의 M-1을 먼저 고쳤을 때, **세 번째
스캐너를 빠뜨렸다** — 독립 스킬 경로(`sources/skills.ts`의 `readSkillDir`)는 `lstat` 없이
`SKILL.md`를 바로 `readFileSync`했다. `SKILL.md` 자리에 FIFO를 심으면 그 호출이 **영구
블록**되고, 이 스캐너를 타는 세 도달 경로가 함께 멈춘다 — `ctk scan`(`collectSkills`) ·
`ctk gen`(`findSkillDirsById`) · `ctk web`의 자산 상세 조회(`classifyAssetDocState`, 동기
읽기 + 단일 이벤트 루프라 **서버 전체가 멈춘다**).

- **관측 방법**: 별도 프로세스로 스캐너를 실행하고 `spawnSync`로 **하드 데드라인**을 건
  뒤 `SKILL.md` 자리에 FIFO(`mkfifo`)를 심어 타임아웃(`SIGKILL`, `EXIT=137`)이 뜨는지 본다.
  ⚠️ **같은 프로세스에서는 관측·검증이 불가능하다** — 동기 블로킹 읽기가 이벤트 루프를 멈추므로
  그 프로세스 안에서 타임아웃을 걸 방법이 없다. 별도 프로세스 + 하드 데드라인이 유일한 축이다.
- **처방**: 세 스캐너의 판정을 `readForFrontmatterScan`(`probe/src/frontmatter-scan.ts`) 단일
  관문으로 모았다 — `statSync().isFile()`로 일반 파일이 아닌 것(FIFO·소켓·디바이스, 링크를
  따라간 최종 대상 기준)을 **열기 전에** 배제한다. `lstat`이 아니라 `stat`을 쓴 이유는 별도
  실측 사실(아래 이어지는 항목)에 있다.
- **파급**: 보안 심사 지적을 "파일과 줄번호"로 좁게 지시하면 **그 자리만 고치고 범위를 놓친다**
  (CLAUDE.md 안전 원칙 5). 독립 스킬은 번들보다 모집단이 크고 도달 경로가 셋이었다.

## frontmatter 스캔 절삭이 자산의 `name` 판정 자체를 바꿀 수 있다 (B1 보안 심사 3차 L-A, 2026-08-27 실측)

`parseSimpleFrontmatter`(`probe/src/frontmatter.ts`)는 닫는 `---`가 없으면 **파일 끝까지
소비하며 나중에 나온 같은 키로 덮어쓴다**(last-write-wins). frontmatter 스캔에는 64KB 상한
(`FRONTMATTER_SCAN_MAX_BYTES`, `probe/src/frontmatter-scan.ts`)이 있어 그보다 큰 파일은
**앞부분만 잘라 읽는다.**

- **실측**: 상한 밖(64KB 이후)에 두 번째 `name:` 줄을 심으면, **잘린 읽기와 안 잘린 읽기가
  서로 다른 `name`을 낸다.** 3차 심사 이전 주석은 "닫는 구획자가 없으면 빈 결과로 자연히
  처리된다"고 **주장**했으나 이 실측이 그것을 반증했다.
- **파급**: **잘림이 판정에 영향을 준다** — "이 파일이 자칭하는 이름"이 상한 유무에 따라
  달라지므로, 절삭된 파일의 frontmatter 판정은 신뢰할 수 없다. `readForFrontmatterScan`은
  `truncated: true`를 반환하고, 호출자는 이 플래그를 **삼키지 않고 건수를 세어**
  (`oversizeTruncated`) 노출해야 한다 — 삼키면 "없음"과 "잘려서 못 잼"이 같아진다(안전 원칙 7).
- **처방 방향**: 닫는 구획자 기준으로 자르거나(파서를 스트리밍으로 바꿔야 함, 이번엔 안 함),
  그 파일의 판정을 `unmeasured`로 남긴다. **B4 후속(L-A)으로 남아 있다** — 현재는 절삭 사실을
  건수로만 노출하고 판정 자체는 그대로 신뢰한다.

## 중첩 커맨드(`commands/<subdir>/*.md`)는 실재하지만 호출 이름은 미측정이다 (2026-08-27 실측)

플러그인 번들 `commands/` 아래 하위 디렉터리를 가진 사례가 **실재한다** — `scanFlatMdKind`는
그 디렉터리를 순회는 하되(`nestedUnmeasured` 카운터로만 존재를 센다), **하위 툴로 편입하지
않는다.** 실측 범위 밖으로 명시하고 미측정으로 남겼다(B1 계획서 §「범위 밖」).

- **관측 방법**: `commands/`의 top-level `Dirent.isDirectory()` 사례를 전수 세고, 이 머신의
  세션 기록(`session_search`)에서 그 중첩 경로가 어떤 이름으로 스폰됐는지 조회한다.
- **결과**: 평면 커맨드(`commands/*.md`)는 **`/plugin:command` 형태로 호출 기록에 남는 것을
  확인했다.** 중첩 커맨드(`commands/<subdir>/*.md`)가 `/plugin:subdir:command`인지,
  `/plugin:command`(하위 디렉터리 생략)인지, 아니면 다른 규약인지는 **이 머신 세션 기록에
  호출 흔적이 0건이라 확정할 수 없었다.**
- **파급**: 편입 건수·파서 설계가 이 규약에 의존한다 — 잘못 추측하면 두 커맨드가 같은 id로
  충돌하거나 계층 부모가 잘못 배정될 수 있다. **추측으로 넣지 않고 `unmeasured`로 남긴다**
  (CLAUDE.md: "추측이나 도움말 인용만으로는 넣지 않는다"). B4 이전 그 규약을 다루는 사람은
  더 넓은 세션 표본에서 먼저 재측정해야 한다.

## B4-c — `docs/workflow-assets.md` 부분 자동 생성 (2026-08-31 실측)

⚠️ **public 위생**: 아래 수치는 **표에 이미 커밋된 21자산**에 대한 것만 절대값으로 적고, 카탈로그
전체 규모·설치 목록은 **비율·상대 표현**으로만 쓴다(CLAUDE.md 「저장소 경계」).

### 조회 키 — `(kind, name)`은 이 머신에서 유일하지 않다

- **관측**: 표의 21자산 중 **5건**이 `(kind, name)` 2튜플로는 후보가 둘 이상이다(최대 4).
  `(kind, pluginName, name)` 3튜플로 바꾸면 **전부 정확히 1건**이 된다.
  실측 대조 — 2튜플 `[4,4,2,2,2]` → 3튜플 `[1,1,1,1,1]`.
- **관측 방법**: 카탈로그 인덱스를 `buildTupleIndex`에 태워 두 축의 후보 수를 나란히 센다
  (`packages/core/src/workflow-doc/index-3tuple.ts`). 키 문자열을 하드코딩하지 않고 `tupleKey()`를 거친다.
- **파급**: 2튜플로 짰다면 첫 `--check`가 `ambiguous`로 **exit 3**이 되어 커맨드가 아예 못 돈다.
  충돌 상대가 하필 그 문서의 「쓰지 않는 것」 절이 명시한 플러그인들이다.
  ⚠️ **정본(ROADMAP·계획서)이 "19건은 2튜플로 붙는다"고 적고 있었다** — 그 수는 플러그인 축으로
  이미 필터한 뒤의 값이었다. **착수 전에 정본의 전제를 코드로 확인한다**가 여기서 값을 했다.
- **선례**: `packages/core/src/usage/subagent-resolve.ts`(B4-b)가 같은 문제를 먼저 풀었다 —
  2단 맵 · `pluginNameFromId`(마지막 `@`) · "후보가 정확히 하나일 때만 잇는다" ·
  **같은 플러그인 안에서도 2건일 수 있음.** 알고리즘은 계승하되 입력을 `Asset[]`이 아니라
  **인덱스 행**으로 바꾼다(인덱스에는 `description`이 없어 열거 위험이 낮다).

### 문서 표기와 카탈로그 키가 어긋나는 축 — 오늘 정확히 2건

| 문서 표기 | 카탈로그 | 어긋난 축 |
|---|---|---|
| `Skill(oh-my-claudecode:plan)` | name이 `omc-plan` | **이름** — 카탈로그는 자칭 frontmatter `name`을 쓴다 |
| `Skill(claude-md-management:revise-claude-md)` | kind가 `command` | **유형** — 디스크가 `commands/*.md`다 |

나머지 19건은 `(kind, plugin, name)`으로 정확히 붙는다. omc 안에 `debug`·`verify`가 command와
skill 양쪽에 있으나 **문서가 `Skill(...)`로 적어 둔 덕에 kind가 모호성을 해소한다.**

### 카탈로그 인덱스에는 `description`이 없다

- **관측**: `catalog/index.json` 항목의 필드는 `id · kind · name · parent_asset_id? ·
  gen_state? · gen_content_sha256?`뿐이다. `description`은 자산별 `asset.json`에 있다.
- **파급**: 조회가 **2단**이 된다(인덱스 → id → `assetJsonPath` → `asset.json`). 동시에 이것이
  "열거하지 않는다"의 근거이기도 하다 — 인덱스만으로는 설명이 나오지 않으므로, 화이트리스트로
  좁힌 자산에 대해서만 본문을 읽으면 된다.
- ⚠️ **첫 측정이 전부 `(없음)`이었다** — 인덱스에서 `description`을 찾았기 때문이다.
  **결과가 전부 같거나 전부 0이면 대상이 아니라 도구를 의심한다.**

### 설명 길이 모집단 (표의 21자산)

최대 **172자** · 중앙값 **73** · 최소 **49**. 자산 3개를 담은 최대 행의 합은 **329자**.
⚠️ **오늘의 표본은 자산당 상한(200)에도 셀 총합 상한(400)에도 닿지 않는다** — 절단·공정 분배
경로는 **실제 데이터로 태울 수 없고 합성 주입으로만 검증된다.** "미측정"이 아니라 "모집단이
상한 아래"이지만, 상류 설명이 길어지면 그때 처음 도는 코드다.

### 마크다운 표 — GFM에서 백틱은 `|`를 보호하지 않는다

- **관측**: `` | a | `x|y` | b | ``는 **4셀로 렌더된다.** 셀 안에 `|`를 넣는 유일한 방법은 `\|`다.
- **파급**: "백틱 안 파이프는 무시"하는 직관적 구현은 **렌더러와 다르게 세어** 셀 경계 판정이
  어긋난다. 경계 스캐너는 백틱을 보지 않아야 한다.
- **관련**: 표 행 `| a | b |`에서 **마지막 `|`는 행을 닫는 파이프**다. "마지막 `|` 이후"를
  치환 구간으로 잡으면 **빈 문자열**을 치환하게 된다 — 치환할 곳은 **마지막 두 파이프 사이**다.

### JS `\s`는 ESC·BEL을 매치하지 않는다

- **관측**: `\s`는 공백·탭·개행류만 본다 — `U+001B`(ESC)·`U+0007`(BEL)에 매치하지 않는다.
- **파급**: 마크다운 이스케이프(`& \ | < >`)와 공백 접기만으로는 **터미널 제어문자·bidi
  오버라이드·제로폭**이 통과한다. 서드파티 원문을 문서나 터미널로 내보내는 경로는 그 축을
  **별도로 제거**해야 한다(OSC 8 하이퍼링크 위장이 실증됐다).
- ⚠️ **제거 범위에 넣으면 안 되는 것이 둘 있다** — **U+200D(ZWJ)**는 지우면 ZWJ 이모지가
  깨지고, **U+200C(ZWNJ)**는 페르시아어·힌디어 정서법에 필수다(빼면 단어 의미가 바뀐다).
  거꾸로 **U+061C**는 RLM의 아랍어 대응이라 Bidi_Control을 덮으면서 빠뜨리기 쉽다.

### `docs/**`의 위생 축 — 경로 표기는 정상이고 설치 목록 축이 무방비였다

- **관측**: `docs/*.md`·`README.md`·`ROADMAP.md`에서 물결 홈 표기는 **27건**이고 전부
  `~/.claude.json` 같은 정당한 설정 경로 관습이다(사용자명 없음). 마켓플레이스 자산 id 형태는
  **4회 출현 / 고유 2개**이고 둘 다 형식을 서술한 자리표시자다. machine_id·스냅샷 파일명은 0건.
- **파급**: 문서 전문에 **경로 축까지 걸면 오늘 통과하는 파일이 새로 빨개진다**(과잉 차단).
  축을 **호출부가 선언**하게 하고 문서 전문은 설치 목록 축만, 생성 산출물·화면 출력은 전 축을 본다.
- ⚠️ **`scripts/hygiene-check.mjs`가 경로 축을 본다는 근거는 부분적으로만 성립한다** — 그 스크립트와
  `findRawPathLeaks`의 정규식이 다르고(부정 lookbehind 유무), "실행 머신의 홈·hostname 리터럴" 축은
  **CI 러너에서는 아무것도 지키지 않는다**(그 머신 값으로 계산되기 때문). **정본의 전제도 축이다.**
