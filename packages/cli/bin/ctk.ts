    ...
    switch (command) {
      ...
      case "agent-probe": {
        ...
        if (catalog === undefined || query === undefined) {
          console.error(
            '사용법: ctk agent-probe --catalog <합성 카탈로그 루트> --query "<질의>" --max-budget-usd <수치> --timeout-sec <초>',
          );
          console.error("  --catalog 는 catalog/index.json 을 담은 디렉터리의 **부모**다(예: fixtures/agent-probe-catalog).");
          process.exitCode = 1;
          return;
        }
