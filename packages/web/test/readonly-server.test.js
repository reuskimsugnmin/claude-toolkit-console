...
import { LOOPBACK_HOST, matchRoute, startReadonlyServer } from "../server/app.js";
/**
...
const DOCS = { "synth-a::annotation": "# 합성 주석\n" };
let running = null;
...
describe("응답 내용", () => {
    ...
    it("/api/usage는 사용량 뷰를 순위 자격과 함께 준다", async () => {
        const server = await start();
        const body = (await (await fetch(`${server.url}/api/usage`)).json());
