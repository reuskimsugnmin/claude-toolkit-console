...
import { createBundledToolLocationCache, findBundledToolPath } from "../src/sources/bundled.js";
...
describe("probe/sources/bundled — BundledToolLocationCache (보안 심사 M-3)", () => {
  ...
  it("같은 캐시를 재사용하면 두 번째 호출은 디스크를 다시 읽지 않는다 — 신규 에이전트가 추가돼도 안 보인다", () => {
    ...
    const first = findBundledToolPath(fixture.home, parentId, "agent", "one", cache);
    ...
    const secondWithSharedCache = findBundledToolPath(fixture.home, parentId, "agent", "two", cache);
    ...
    const withFreshCache = findBundledToolPath(fixture.home, parentId, "agent", "two", freshCache);
  ...
  it("캐시를 생략하면(기본 인자) 매 호출이 새로 읽는다 — 캐시 없음과 동형(회귀 없음)", () => {
    ...
    expect(findBundledToolPath(fixture.home, parentId, "agent", "one")).toHaveLength(1);

    writeFlatMd(pluginDir, "two.md", "두 번째 에이전트");
    // 캐시를 넘기지 않은 두 번째 호출은 디스크를 다시 읽으므로 새로 추가된 파일도 바로 보인다.
    expect(findBundledToolPath(fixture.home, parentId, "agent", "two")).toHaveLength(1);
  });
  ...
  it("한 캐시로 같은 부모의 서로 다른 자식(agent·command·skill)을 순서대로 찾아도 결과가 서로 섞이지 않는다", () => {
    ...
    const cache = createBundledToolLocationCache();
    const agent = findBundledToolPath(fixture.home, parentId, "agent", "the-agent", cache);
    const command = findBundledToolPath(fixture.home, parentId, "command", "the-command", cache);
    const skill = findBundledToolPath(fixture.home, parentId, "skill", "the-skill", cache);
