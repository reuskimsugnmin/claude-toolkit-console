...
/**
 * ⚠️ 자칭 name 판정은 **이 파일에 없다** — `./asset-name.ts`로 옮겼다(보안 재심 M-2).
 * private으로 두었더니 `sources/mcp.ts`의 독립 MCP 축이 같은 검사 없이 최상위 JSON 키를
 * 그대로 `Asset.id`로 쓰고 있었다(`validateInstallPath`와 똑같은 형태의 결함).
 */
function safeSuffix(candidate: string): string | null {
  return safeAssetNameSegment(candidate, "번들 하위 툴의 자칭 name");
}
